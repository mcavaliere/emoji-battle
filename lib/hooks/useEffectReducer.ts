// Copy of a forked version of David Khourshid's useEffectReducer hook, with a fix for event duplication:
//  https://github.com/davidkpiano/useEffectReducer/pull/19
...
>(
  ...
  const initialStateAndEffects: AggregatedEffectsState<TState, TEvent> =
    useMemo(() => {
      if (typeof initialState === 'function') {
        const initialEffectEntities: Array<EffectEntity<TState, TEvent>> = [];

        const resolvedInitialState = (
          initialState as InitialEffectStateGetter<TState, TEffect>
        )((effect) => {
          const effectObject = toEffectObject(effect, effectsMap);
          const effectEntity = createEffectEntity<TState, TEvent, TEffect>(
            effectObject
  ...
  const [[state, effectStateEntityTuples, entitiesToStop], dispatch] =
